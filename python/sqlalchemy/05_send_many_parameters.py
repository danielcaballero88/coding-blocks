...
def main():
    ...
    #
    # send multiple parameters with bound parameters to an insert 
    with engine.connect() as conn:
        conn.execute(
            text('INSERT INTO some_table (x, y) VALUES (:x, :y)'), # note the bound parameter sintax with the ':'
            [{'x': 11, 'y': 12}, {'x':13, 'y': 14}] # and here is a list of dictionaries with the bound parameters values
        )
    # query and print
        result = conn.execute(
