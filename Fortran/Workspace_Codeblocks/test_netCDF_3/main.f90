...
program hello
    ...
    ALLOCATE( r_var1_vals(i_dim1_val, i_dim2_val) )
    ! asignar valores al array en memoria
    DO i=1,i_dim1_val
        DO j=1,i_dim2_val
            r_var1_vals(i,j) = (i-1)*i_dim2_val + j
        END DO
    END DO
    ! asignar valores
    i_error = nf90_put_var(i_fileID, i_var1_id, r_var1_vals, start=(/ 1, 1, 2 /))
    if(i_error /= nf90_NoErr) print*, NF90_STRERROR(i_error)
    ! ---
