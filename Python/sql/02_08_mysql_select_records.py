...
def main():
    ...
    # ---
    # Insert records (using cursor.execute() and cursor.executemany())
    insert_records(dbConnection, tableName, fieldNames, fieldValues)
    insert_many_records(dbConnection, tableName, fieldNames, fieldValues*10)
    # ---
    # Select records
    select_records(dbConnection, tableName, offset=5, limit=5)
    # Select records where
    select_records_where(dbConnection, tableName, offset=5, limit=5, where='b = 2')
...
def select_records(dbConn, tableName, offset, limit):
    print('---')
    print('Trying to select records from table "%s" in database "%s"' % (tableName, get_database_name(dbConn)))
    query = f'SELECT * FROM {tableName} LIMIT {offset},{limit};'
    print(query)
    try:
        cursor = dbConn.cursor()
        cursor.execute(query)
...
def select_records_where(dbConn, tableName, offset, limit, where):
    print('---')
    print('Trying to select records from table "%s" in database "%s"' % (tableName, get_database_name(dbConn)))
