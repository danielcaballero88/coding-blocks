...
!
!in order to acomplish that, some information must be given under a flag in BuildInfo.txt:
!*TURBO INTERFACER
!Archivo de malla de entrada (tipicamente BuildMesh.txt)
...
!
!IMPORTANT: an output mesh file must be passed as argument, e.g., "BuildMeshFSI.txt"
!this is the name that must appear under the flag *MESH FILE in BuildInfo.txt
!and it must differ from the output mesh file name
SUBROUTINE turbointerfacer(OutMeshFile, NDim)

    IMPLICIT REAL*8 (A-H,O-Z)
    INTEGER, INTENT(IN) :: OutMeshFile
    INTEGER, INTENT(IN)           :: NDim

    Character DummyString*120, Str*120, NelName*12,InMeshFileName*60, OutMeshFileName*60
    ...
    Integer*4 FindStringInFile,InfoFile
    ...
    OPEN ( InfoFile, FILE= 'BuildInfo.txt' )

    REWIND(InfoFile)
    Str='*TURBO INTERFACER'
    iError = FindStringInFile(Str,InfoFile)
    if (iError.ne.0) then
        Return
    End if
    WRITE(*,*) TRIM(Str)," found in BuildInfo.txt, trying to create Prism6 FSI coupling group from shared Tri3 surface"
    READ(InfoFile,"(30A)") InMeshFileName

    INQUIRE( UNIT=OutMeshFile, NAME=OutMeshFileName )

    IF (InMeshFileName .EQ. OutMeshFileName) THEN
        WRITE(*,*) "INPUT AND OUTPUT MESH FILES FOR TURBO INTERFACER ARE THE SAME"
        WRITE(*,*) "STOP, ERROR"
        STOP
    END IF

    OPEN ( MeshFile,  FILE= InMeshFileName )
    !OPEN ( OutMeshFile, FILE= OutMeshFileName )
    rewind(OutMeshFile)
